...
Additional supplemental data not present in preprint version
```{r}
#CAP Analysis (supplemental figure 1-200 lines)
...
alleles<- read.csv("PInumber.csv", sep=",", header=TRUE) #added in Alleles in excel afterwards, then reimported
sam.new <- sample_data(alleles)
head(sam.new)
rownames(sam.new) <- sample(sample_names(rar_trans))
ps0 <- merge_phyloseq(rar_trans, sample_data(sam.new))
head(sample_data(ps0))
ordcap200 = ordinate(ps0, "CAP", "bray", ~Alleles)

newplot <- phyloseq::plot_ordination(ps0,ordcap200,color="Alleles",shape="Alleles") +
  geom_point(size=4) +
  scale_color_manual(values=c("#A9A9A9", "#a04344")) +
...
#perform permanova
ps.prop_dist <- distance(ps0, method = "bray")
ps.prop_data <- data.frame(sample_data(ps0))
